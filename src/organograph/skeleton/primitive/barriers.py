"""Soft-barrier body and branch primitives used by skeletonization.

The fitted ellipsoid or superellipsoid is intended as an initial body/branch
representation shared by component assignment and primitive fitting. It treats the mesh as a
...
@dataclass
class BarrierPrimitiveConfig:
    """Settings for PCA-frame soft-barrier primitive fitting.
...
def barrier_primitive_level(points, fit: BarrierPrimitiveFit) -> np.ndarray:
...
def fit_barrier_primitive_sampled(
    vertices,
    faces=None,
    *,
    sample_fraction: float = 1.0,
    ...
    random_seed: int | None = 0,
    weights=None,
    config: BarrierPrimitiveConfig | dict[str, Any] | None = None,
    require_inside_center: bool = True,
    center0=None,
    axes=None,
) -> BarrierPrimitiveFit:
    """Fit a soft-barrier primitive on a sampled vertex cloud.
...
def relative_height_field(vertices, fit: BarrierPrimitiveFit) -> dict[str, np.ndarray]:
...
def fit_barrier_primitive(
    ...
    *,
    ...
    if not isinstance(config, BarrierPrimitiveConfig):
        config = BarrierPrimitiveConfig(**dict(config or {}))
...
def fit_branch_barrier_primitives(
    ...
    *,
    ...
    fits: dict[str, BarrierPrimitiveFit] = {}
...
def exclude_host_vertices_from_patches(
    patches,
    protected_mask,
    *,
    min_vertices: int = 1,
    ...
    This keeps the body estimate from being assigned to crypt candidates
    before later refinement and neckline computations.
    """
...
def exclude_host_vertices_from_detections(
    detections: list[dict[str, Any]],
    protected_mask,
    *,
    recursive: bool = True,
    ...
    This does not alter distance fields or already-computed neck positions; it
    prevents component extraction and graph construction from treating
    protected body/branch vertices as crypt mesh.
    """
    protected = np.asarray(protected_mask, dtype=bool).reshape(-1)
    allowed = ~protected
