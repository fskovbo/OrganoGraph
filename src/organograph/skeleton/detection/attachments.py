...
def find_barrier_boundary_crossing(
    ...
    *,
    ...
    max_axis_level: float | None = None,
    n_samples: int = 40,
    persistence: int = 2,
    ...
    Ring centers are sampled across the available finite geodesic domain unless
    an explicit upper level is supplied. A
    ...
    diagnostics: dict[str, Any] = {
        ...
        "max_axis_level": None if max_axis_level is None else float(max_axis_level),
    ...
    if max_axis_level is not None:
        hi = min(float(max_axis_level), hi)
...
def attachment_crossing_diagnostics(
    detections: list[dict[str, Any]],
    *,
    metadata_key: str = "barrier_boundary_crossing",
) -> list[dict[str, Any]]:
    """Flatten attachment-crossing diagnostics for batch quality control."""
    records = []

    def collect(det, *, parent_id=None):
        crypt_id = det.get("crypt_id")
        crossing = dict(det.get("metadata", {}).get(metadata_key, {}))
        records.append(
            {
                "crypt_id": crypt_id,
        ...
        for daughter in det.get("daughters") or []:
            collect(daughter, parent_id=crypt_id)
